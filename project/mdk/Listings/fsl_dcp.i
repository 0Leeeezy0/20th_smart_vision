# 1 "../../libraries/sdk/drives/fsl_dcp.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 387 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "../../libraries/sdk/drives/fsl_dcp.c" 2
...
# 1 "../../libraries/sdk/drives/fsl_dcp.h" 1
# 12 "../../libraries/sdk/drives/fsl_dcp.h"
...
# 13 "../../libraries/sdk/drives/fsl_dcp.h" 2
# 62 "../../libraries/sdk/drives/fsl_dcp.h"
enum _dcp_status
{
    kStatus_DCP_Again = ((((kStatusGroup_DCP)*100L) + (0))),
};
...
{
    _Bool gatherResidualWrites;
    _Bool enableContextCaching;
    _Bool enableContextSwitching;
    uint8_t enableChannel;
    uint8_t enableChannelInterrupt;
} dcp_config_t;
# 209 "../../libraries/sdk/drives/fsl_dcp.h"
...
status_t DCP_AES_EncryptCbc(DCP_Type *base,
                            ...
                            size_t size,
                            const uint8_t iv[16]);
# 391 "../../libraries/sdk/drives/fsl_dcp.h"
status_t DCP_AES_DecryptCbc(DCP_Type *base,
                            ...
                            size_t size,
                            const uint8_t iv[16]);
# 420 "../../libraries/sdk/drives/fsl_dcp.h"
status_t DCP_AES_EncryptEcbNonBlocking(DCP_Type *base,
                                       ...
                                       uint8_t *ciphertext,
                                       size_t size);
# 441 "../../libraries/sdk/drives/fsl_dcp.h"
status_t DCP_AES_DecryptEcbNonBlocking(DCP_Type *base,
                                       ...
                                       uint8_t *plaintext,
                                       size_t size);
# 463 "../../libraries/sdk/drives/fsl_dcp.h"
status_t DCP_AES_EncryptCbcNonBlocking(DCP_Type *base,
                                       ...
                                       size_t size,
                                       const uint8_t *iv);
# 486 "../../libraries/sdk/drives/fsl_dcp.h"
...
status_t DCP_HASH(DCP_Type *base,
                  ...
                  uint8_t *output,
                  size_t *outputSize);
# 10 "../../libraries/sdk/drives/fsl_dcp.c" 2
...
# 12 "../../libraries/sdk/drives/fsl_dcp.c" 2
# 29 "../../libraries/sdk/drives/fsl_dcp.c"
...
{
    kDCP_CONTROL0_DECR_SEMAPHOR = 1u << 1,
    kDCP_CONTROL0_ENABLE_HASH = 1u << 6,
    kDCP_CONTROL0_HASH_INIT = 1u << 12,
    kDCP_CONTROL0_HASH_TERM = 1u << 13,
...
__attribute__((section("NonCacheable.init"))) static dcp_context_t s_dcpContextSwitchingBuffer;
...
{
    while (0U != ((uint32_t)dcpWorkExt & ((uint32_t)(32) - 1U)))
    {
        dcpWorkExt++;
    }
...
static status_t dcp_get_channel_status(DCP_Type *base, dcp_channel_t channel)
{
    ...
    if (!((0U != (semaReg & (0xFF0000U))) || (0U != (statReg & (0xFF0000U)))))
    {
...
{
    base->KEY = (((uint32_t)(((uint32_t)(handle->keySlot)) << (4U))) & (0x30U)) | (((uint32_t)(((uint32_t)(0)) << (0U))) & (0x3U));
...
static status_t dcp_schedule_work(DCP_Type *base, dcp_handle_t *handle, dcp_work_packet_t *dcpPacket)
{
    ...
    if ((base->STAT & (uint32_t)handle->channel) != (uint32_t)handle->channel)
    {
        ...
        {
            volatile uint32_t *cmdptr = 0;
            volatile uint32_t *chsema = 0;
...
# 316 "../../libraries/sdk/drives/fsl_dcp.c"
status_t DCP_AES_SetKey(DCP_Type *base, dcp_handle_t *handle, const uint8_t *key, size_t keySize)
{
    status_t status = kStatus_Fail;
    ...
    if ((kDCP_OtpKey == handle->keySlot) || (kDCP_OtpUniqueKey == handle->keySlot))
    {
        ...
        if ((base->STAT & (0x10000000U)) == (0x10000000U))
        {
            status = kStatus_Success;
        }
    }
...
# 379 "../../libraries/sdk/drives/fsl_dcp.c"
status_t DCP_AES_EncryptEcb(
    DCP_Type *base, dcp_handle_t *handle, const uint8_t *plaintext, uint8_t *ciphertext, size_t size)
{
    ...
    uint8_t dcpWorkExt[sizeof(dcp_work_packet_t) + (32)] = {0U};
...
# 421 "../../libraries/sdk/drives/fsl_dcp.c"
status_t DCP_AES_EncryptEcbNonBlocking(DCP_Type *base,
                                       dcp_handle_t *handle,
                                       dcp_work_packet_t *dcpPacket,
                                       const uint8_t *plaintext,
                                       ...
                                       size_t size)
{
    ...
    dcpPacket->control0 =
        0x122u | (handle->swapConfig & 0xFC0000u);
    ...
    {
        dcpPacket->control0 |= ((uint32_t)1u << 10);
        dcpPacket->control1 = ((uint32_t)0xFFu << 8);
    }
    else if (handle->keySlot == kDCP_OtpUniqueKey)
    {
        dcpPacket->control0 |= ((uint32_t)1u << 10);
        dcpPacket->control1 = ((uint32_t)0xFEu << 8);
    }
    else if (handle->keySlot == kDCP_PayloadKey)
    {

        dcpPacket->payloadPointer = (uint32_t)&handle->keyWord[0];
        dcpPacket->control0 |= ((uint32_t)1u << 11);
    }
    else
    {
        dcpPacket->control1 = ((uint32_t)handle->keySlot << 8);
    }

    return dcp_schedule_work(base, handle, dcpPacket);
}
# 477 "../../libraries/sdk/drives/fsl_dcp.c"
status_t DCP_AES_DecryptEcb(
    DCP_Type *base, dcp_handle_t *handle, const uint8_t *ciphertext, uint8_t *plaintext, size_t size)
{
    status_t completionStatus = kStatus_Fail;
...
# 519 "../../libraries/sdk/drives/fsl_dcp.c"
status_t DCP_AES_DecryptEcbNonBlocking(DCP_Type *base,
                                       dcp_handle_t *handle,
                                       dcp_work_packet_t *dcpPacket,
                                       const uint8_t *ciphertext,
                                       ...
                                       size_t size)
{
    ...
    dcpPacket->control0 = 0x22u | (handle->swapConfig & 0xFC0000u);
...
# 575 "../../libraries/sdk/drives/fsl_dcp.c"
status_t DCP_AES_EncryptCbc(DCP_Type *base,
                            dcp_handle_t *handle,
                            const uint8_t *plaintext,
                            uint8_t *ciphertext,
...
# 622 "../../libraries/sdk/drives/fsl_dcp.c"
status_t DCP_AES_EncryptCbcNonBlocking(DCP_Type *base,
                                       dcp_handle_t *handle,
                                       dcp_work_packet_t *dcpPacket,
                                       const uint8_t *plaintext,
                                       ...
                                       const uint8_t *iv)
{
    ...
    dcpPacket->control0 =
        0x322u | (handle->swapConfig & 0xFC0000u);
    dcpPacket->control1 = 0x10u;
    dcpPacket->sourceBufferAddress = (uint32_t)plaintext;
    dcpPacket->destinationBufferAddress = (uint32_t)ciphertext;
    dcpPacket->bufferSize = (uint32_t)size;
    ...
    else if (handle->keySlot == kDCP_PayloadKey)
    {
        ...
        (void)memcpy(handle->iv, (const uint32_t *)(uintptr_t)iv, 16);
        dcpPacket->payloadPointer = (uint32_t)&handle->keyWord[0];
        dcpPacket->control0 |= ((uint32_t)1u << 11);
    }
    else
...
# 686 "../../libraries/sdk/drives/fsl_dcp.c"
status_t DCP_AES_DecryptCbc(DCP_Type *base,
                            dcp_handle_t *handle,
                            const uint8_t *ciphertext,
                            uint8_t *plaintext,
...
# 733 "../../libraries/sdk/drives/fsl_dcp.c"
status_t DCP_AES_DecryptCbcNonBlocking(DCP_Type *base,
                                       dcp_handle_t *handle,
                                       dcp_work_packet_t *dcpPacket,
                                       const uint8_t *ciphertext,
                                       ...
                                       const uint8_t *iv)
{
    ...
    dcpPacket->control0 = 0x222u | (handle->swapConfig & 0xFC0000u);
    dcpPacket->control1 = 0x10u;
    dcpPacket->sourceBufferAddress = (uint32_t)ciphertext;
    dcpPacket->destinationBufferAddress = (uint32_t)plaintext;
    dcpPacket->bufferSize = (uint32_t)size;
...
# 795 "../../libraries/sdk/drives/fsl_dcp.c"
void DCP_GetDefaultConfig(dcp_config_t *config)
{
    ...
    dcp_config_t userConfig = {
        1, 0, 1, (uint8_t)kDCP_chEnableAll, (uint8_t)kDCP_chIntDisable,
    };

    *config = userConfig;
}
# 820 "../../libraries/sdk/drives/fsl_dcp.c"
void DCP_Init(DCP_Type *base, const dcp_config_t *config)
{
    ...
    base->CTRL = 0xF0800000u;
    base->CTRL = 0x30800000u;
    ...
    base->CTRL = (((uint32_t)(((uint32_t)(config->gatherResidualWrites)) << (23U))) & (0x800000U)) |
                 (((uint32_t)(((uint32_t)(config->enableContextCaching)) << (22U))) & (0x400000U)) |
                 (((uint32_t)(((uint32_t)(config->enableContextSwitching)) << (21U))) & (0x200000U)) |
                 (((uint32_t)(((uint32_t)(config->enableChannelInterrupt)) << (0U))) & (0xFFU));


    base->CHANNELCTRL = (((uint32_t)(((uint32_t)(config->enableChannel)) << (0U))) & (0xFFU));


    base->CONTEXT = (uint32_t)&s_dcpContextSwitchingBuffer;
}
# 852 "../../libraries/sdk/drives/fsl_dcp.c"
void DCP_Deinit(DCP_Type *base)
{
    base->CTRL = 0xF0800000u;
...
# 872 "../../libraries/sdk/drives/fsl_dcp.c"
status_t DCP_WaitForChannelComplete(DCP_Type *base, dcp_handle_t *handle)
{
...
# 898 "../../libraries/sdk/drives/fsl_dcp.c"
...
# 918 "../../libraries/sdk/drives/fsl_dcp.c"
...
# 943 "../../libraries/sdk/drives/fsl_dcp.c"
static status_t dcp_hash_check_context(dcp_hash_ctx_internal_t *ctxInternal, const uint8_t *message)
{
    if ((0 == message) || (0 == ctxInternal) || (kStatus_Success != dcp_hash_check_input_alg(ctxInternal->algo)))
    {
        return kStatus_InvalidArgument;
    }
...
# 961 "../../libraries/sdk/drives/fsl_dcp.c"
...
# 1063 "../../libraries/sdk/drives/fsl_dcp.c"
                                              ...
                                              size_t messageSize)
{
    ...
    {
        size_t toCopy = 128U - ctxInternal->blksz;
        (void)memcpy(&ctxInternal->blk.b[ctxInternal->blksz], message, toCopy);
        message += toCopy;
        messageSize -= toCopy;


        status = dcp_hash_update(base, ctxInternal, &ctxInternal->blk.b[0], 128U);
    ...
    uint32_t fullBlocksSize = ((messageSize >> 6) << 6);
    ...
    (void)memcpy(&ctxInternal->blk.b[0], message, messageSize);
    ctxInternal->blksz = messageSize;

    return status;
}
# 1115 "../../libraries/sdk/drives/fsl_dcp.c"
...
static void dcp_hash_save_running_hash(dcp_hash_ctx_internal_t *ctxInternal)
{
    ...
    if (srcAddr != 0)
    {
        ...
        (void)memcpy(ctxInternal->runningHash, srcAddr, sizeof(ctxInternal->runningHash));
    }
}
...
static void dcp_hash_restore_running_hash(dcp_hash_ctx_internal_t *ctxInternal)
{
    ...
    if (destAddr != 0)
    {
        (void)memcpy(destAddr, ctxInternal->runningHash, sizeof(ctxInternal->runningHash));
    }
}
# 1201 "../../libraries/sdk/drives/fsl_dcp.c"
status_t DCP_HASH_Init(DCP_Type *base, dcp_handle_t *handle, dcp_hash_ctx_t *ctx, dcp_hash_algo_t algo)
{
    status_t status;
    ...
    ctxInternal = (dcp_hash_ctx_internal_t *)(uint32_t)((uint8_t *)ctx + (32));





    extern int dcp_hash_ctx_t_size[1 - 2 * (!(sizeof(dcp_hash_ctx_t) >= sizeof(dcp_hash_ctx_internal_t)))] __attribute__((unused));
    uint32_t i;
...
# 1253 "../../libraries/sdk/drives/fsl_dcp.c"
status_t DCP_HASH_Update(DCP_Type *base, dcp_hash_ctx_t *ctx, const uint8_t *input, size_t inputSize)
{
    _Bool isUpdateState;
    status_t status;
    ...
    {
        (void)memcpy((&ctxInternal->blk.b[0]) + ctxInternal->blksz, input, inputSize);
...
# 1323 "../../libraries/sdk/drives/fsl_dcp.c"
status_t DCP_HASH_Finish(DCP_Type *base, dcp_hash_ctx_t *ctx, uint8_t *output, size_t *outputSize)
{
    size_t algOutSize = 0;
    status_t status;
    ...
    if (ctxInternal->fullMessageSize == 0U)
    {
        ...
        {
            case kDCP_Sha256:
                (void)memcpy(&output[0], &s_nullSha256, 32);
                break;
            case kDCP_Sha1:
                (void)memcpy(&output[0], &s_nullSha1, 20);
                break;
            default:
...
# 1435 "../../libraries/sdk/drives/fsl_dcp.c"
status_t DCP_HASH(DCP_Type *base,
                  dcp_handle_t *handle,
                  dcp_hash_algo_t algo,
                  const uint8_t *input,
