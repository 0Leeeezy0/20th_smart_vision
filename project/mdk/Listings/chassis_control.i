# 1 "../code/chassis_control.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 387 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "../code/chassis_control.c" 2
# 13 "../code/chassis_control.c"
...
void motor_sensor_init(void)
{
 ...
 (pit_init((PIT_CH1), (20) * (CLOCK_GetFreq(kCLOCK_PerClk) / 1000)));


 (pit_init((PIT_CH0), (40) * (CLOCK_GetFreq(kCLOCK_PerClk) / 1000)));
 ...
 pwm_init(PWM2_MODULE1_CHA_C8,10000,0);
 pwm_init(PWM2_MODULE0_CHA_C6,10000,0);
...
# 76 "../code/chassis_control.c"
void motor_set_duty(_MOTOR_NUM_ motor_num,const uint32 duty,uint8 dir)
{
 uint8 dir_fix;
 switch(motor_num)
 {
  case MOTOR_1:{ if(-1 == 1){ if(dir){ dir_fix = 1; }else{ dir_fix = 0; } }else{ if(dir){ dir_fix = 0; }else{ dir_fix = 1; } } break; }
  case MOTOR_2:{ if(1 == 1){ if(dir){ dir_fix = 1; }else{ dir_fix = 0; } }else{ if(dir){ dir_fix = 0; }else{ dir_fix = 1; } } break; }
  case MOTOR_3:{ if(-1 == 1){ if(dir){ dir_fix = 1; }else{ dir_fix = 0; } }else{ if(dir){ dir_fix = 0; }else{ dir_fix = 1; } } break; }
 }
 switch(motor_num)
 {
  case MOTOR_1:{ pwm_set_duty(PWM2_MODULE3_CHB_D3,duty); gpio_set_level(D2,dir_fix); break; }
  case MOTOR_2:{ pwm_set_duty(PWM2_MODULE1_CHA_C8,duty); gpio_set_level(C9,dir_fix); break; }
  case MOTOR_3:{ pwm_set_duty(PWM2_MODULE0_CHA_C6,duty); gpio_set_level(C7,dir_fix); break; }
 }
}
...
void encoder_get(void)
{
 encoder_1_count = encoder_get_count(QTIMER2_ENCODER1);
 encoder_2_count = encoder_get_count(QTIMER1_ENCODER2);
 encoder_3_count = encoder_get_count(QTIMER1_ENCODER1);

 encoder_1_speed = (float)encoder_get_count(QTIMER2_ENCODER1)/(1024*(float)40/1000);
 encoder_2_speed = (float)encoder_get_count(QTIMER1_ENCODER2)/(1024*(float)40/1000);
 encoder_3_speed = (float)encoder_get_count(QTIMER1_ENCODER1)/(1024*(float)40/1000);

 motor_1_speed = (float)encoder_1_speed/(float)(4.4*26/36);
 motor_2_speed = (float)encoder_2_speed/(float)(4.4*26/36);
 motor_3_speed = (float)encoder_3_speed/(float)(4.4*26/36);
}
...
void gyro_get(void)
{
 ...
 if(epoch < (float)(200))
 {
  gyro_x = ((float)(imu660ra_gyro_x) / imu660ra_transition_factor[1]);
  gyro_y = ((float)(imu660ra_gyro_y) / imu660ra_transition_factor[1]);
  gyro_z = ((float)(imu660ra_gyro_z) / imu660ra_transition_factor[1]);

  gyro_x_calibration += gyro_x/(float)(200);
  gyro_y_calibration += gyro_y/(float)(200);
  gyro_z_calibration += gyro_z/(float)(200);

  epoch++;
 }
 else
 {
  gyro_x = ((float)(imu660ra_gyro_x) / imu660ra_transition_factor[1])-gyro_x_calibration;
  gyro_y = ((float)(imu660ra_gyro_y) / imu660ra_transition_factor[1])-gyro_y_calibration;
  gyro_z = ((float)(imu660ra_gyro_z) / imu660ra_transition_factor[1])-gyro_z_calibration;
 }
}
...
void acc_get(void)
{
 ...
 if(epoch < (float)(200))
 {
  ...
  acc_x = ((float)(imu660ra_acc_x) / imu660ra_transition_factor[0]);
  acc_y = ((float)(imu660ra_acc_y) / imu660ra_transition_factor[0]);
  acc_z = ((float)(imu660ra_acc_z) / imu660ra_transition_factor[0]);

  acc_x_calibration += acc_x/(float)(200);
  acc_y_calibration += acc_y/(float)(200);
  acc_z_calibration += acc_z/(float)(200);

  epoch++;
 }
 else
 {
  acc_x = ((float)(imu660ra_acc_x) / imu660ra_transition_factor[0])-acc_x_calibration;
  acc_y = ((float)(imu660ra_acc_y) / imu660ra_transition_factor[0])-acc_y_calibration;
  acc_z = ((float)(imu660ra_acc_z) / imu660ra_transition_factor[0])-acc_z_calibration;
 }
}
...
float real_yaw()
{
 ...
 if(acc_x > 0.005)
 {
  real_yaw = (float)(180*atan(tan)/( 3.1415926535898 ))+180;
 }
 if(acc_x < -0.005)
 {
  if(acc_y > 0.005)
 ...
 if(acc_x >= -0.005 && acc_x <= 0.005 && acc_y > 0.005)
 {
  real_yaw = (float)90;
 }
 if(acc_x >= -0.005 && acc_x <= 0.005 && acc_y < -0.005)
...
{
 _CHASSIS_CONTROL_ chassis_control;
 float x_speed = speed*sinf((float)(( 3.1415926535898 )*yaw/180));
 float y_speed = (float)speed*cosf((float)(( 3.1415926535898 )*yaw/180));

 chassis_control.motor_1_speed = -x_speed;
 chassis_control.motor_2_speed = x_speed*(float)0.5 +y_speed*(float)0.8660254037844;
 chassis_control.motor_3_speed = x_speed*(float)0.5 -y_speed*(float)0.8660254037844;
...
void chassis_control_move(int16 (*p)(_PID_* pid,float motor_speed,float motor_feedback_speed),float yaw,float speed)
{
 ...
 chassis_control.motor_1 = motor_pid(incremental_pid,&chassis_pid,MOTOR_1,chassis_control.motor_1_speed).motor_1;
 chassis_control.motor_2 = motor_pid(incremental_pid,&chassis_pid,MOTOR_2,chassis_control.motor_2_speed).motor_2;
 chassis_control.motor_3 = motor_pid(incremental_pid,&chassis_pid,MOTOR_3,chassis_control.motor_3_speed).motor_3;
