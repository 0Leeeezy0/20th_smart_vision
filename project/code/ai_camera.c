...
static uint8  ai_camera_0_upgrade_flag = 0;	// AI摄像头0 更新标志位
...
//    get_data = uart_read_byte(UART_INDEX);                                      // 接收数据 while 等待式 不建议在中断使用
    ...
    fifo_write_buffer(&uart_data_fifo_ai_camera_0, &get_data, 2);                           // 将数据写入 fifo 中
    
    fifo_data_count = fifo_used(&uart_data_fifo_ai_camera_0);                           // 查看 fifo 是否有数据
    if(fifo_data_count != 0)                                                // 读取到数据了
    {
        fifo_read_buffer(&uart_data_fifo_ai_camera_0, fifo_get_data_ai_camera_0, &fifo_data_count, FIFO_READ_AND_CLEAN);    // 将 fifo 中数据读出并清空 fifo 挂载的缓冲
        uart_write_buffer(AI_CAMERA_0_UART_INDEX, fifo_get_data_ai_camera_0, fifo_data_count);      // 将读取到的数据发送出去
