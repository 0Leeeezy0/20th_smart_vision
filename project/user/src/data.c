...
float path_linear_speed_target[3] = {70,100,120};	// 循迹线速度（缓启动速度 启动最小速度 启动最大速度）（cm/s）
int16 path_start = 95;			// 路径线提取开始高度
int16 path_end = 30;			// 路径线提取结束高度
int16 control_point[2] = {65 ,50};		// 控制点高度（速度：100：72，55）
...
/* 赛道元素参数 */
float circle_path_linear_speed_target = 80;			// 圆环循迹线速度（cm/s）
...
/* AI追踪 */
float track_linear_speed_target = 60;	// 追踪线速度
...
// 位置式 ( 小角度 中角度 大角度 )
float ROTATE_PID[8][5] = {{0.31 ,0 ,0.04 ,25 ,2},{0.34 ,0 ,0.05 ,30 ,2},{0.39  ,0 ,0.06 ,45 ,2},{0.46 ,0 ,0.08 ,55 ,2},{0.60 ,0 ,0.08 ,70 ,2},{0.80 ,0 ,0.08 ,85 ,2},{1 ,0 ,0.08 ,90 ,2},{1 ,0 ,0.08 ,150 ,2}};
#endif
...
//float PATH_PID[6][6] = {{0.39 ,0 ,0.9 ,1 ,18 ,2},{0.405 ,0 ,0.9 ,1 ,20 ,2},{0.415 ,0 ,0.97 ,1.1 ,25 ,2},{0.425 ,0 ,1.345 ,1.2 ,42 ,2},{0.41 ,0 ,1.3 ,1.3 ,46 ,2},{0.405 ,0 ,1.395 ,1.3 ,55 ,2}}; // linear_speed = 80-120	积分限幅和陀螺仪微分项调整需注意
float PATH_PID[4][6] = {{0.39 ,0 ,0.9 ,1 ,18 ,2},{0.415 ,0 ,0.97 ,1.1 ,25 ,2},{0.425 ,0 ,1.345 ,1.2 ,42 ,2},{0.405 ,0 ,1.395 ,1.3 ,55 ,2}}; // linear_speed = 80-120	积分限幅和陀螺仪微分项调整需注意
