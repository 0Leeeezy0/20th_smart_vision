...
/* 圆环 */
uint16 circle_check_y = 55;				// 圆环检测线高度（越大越晚进环）
...
float y_speed_slow_change_rate = 0.001;			// Y缓变速率（越大缓变速越快）
float x_speed_rate = 2.4;						// x速度比例（目标x速度/目标旋转速度）
/* 箱子 */
...
float circle_y_speed_target = 150;				// 目标圆环Y速度
...
float circle_angle_target[2] = {75, 60};		// 出入环目标转动角度
...
//float PATH_PID[4][6] =    {{ 0.90, 0,     0.5,   2,           75,          0.06},
//						  { 1.60, 0,    -2.9,   2,           75,          0.06},
